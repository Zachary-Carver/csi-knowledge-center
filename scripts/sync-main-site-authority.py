#!/usr/bin/env python3
"""Generate CSI canonical-site authority artifacts from the live main websitemap.

The August 26, 2026 audit established a 50-primary-page baseline. The commercial
site may grow, while blog/article and known system routes remain separate
...
ROOT_SITEMAP = "https://cleansceneinvestigators.com/sitemap.xml"
...
NON_PRIMARY_PATHS = {
    "/home": "Duplicate legacy home route; canonical home is /.",
    "/ols/products": "Legacy store system/catalog route; not a designed CSI authority page.",
}
CONTENT_PREFIXES = {
    "/f/": "Legacy blog/article route; indexable supporting content, tracked separately from designed primary pages.",
    "/follow-us/f/": "Blog/article route under the current Follow Us content hub; indexable supporting content, tracked separately from designed primary pages.",
}
...
def normalize(url: str) -> str:
    ...
    return "https://cleansceneinvestigators.com"+path+(f"?{p.query}" if p.query else "")
...
def main() -> int:
    ...
    payload={"name":"CSI live canonical main-site registry","source":ROOT_SITEMAP,"generatedAt":generated_at,"historicalAuditedBaseline":{"date":BASELINE_DATE,"primaryPageCount":BASELINE_PRIMARY_COUNT},"observedSitemapUrlCount":len(observed),"primaryPageCount":len(primary),"supportingContentAndSystemRouteCount":len(excluded),"canonicalUrls":primary,"observedSitemapUrls":observed,"excludedFromPrimaryArchitecture":excluded,"changeFromCheckedInBaseline":{"added":added,"removed":removed},"authorityRule":"Use canonicalUrls as the current live main-site registry. Supporting blog routes and known system/duplicate routes remain crawlable authority but are tracked separately. Never invent city or service URLs."}
